...
function handleTurn() {
	...
	if (winner === 'p') {
		textEls.pWin.innerText = `You win this round! Collect both cards.`;
	} else {
		textEls.cWin.innerText = `The computer wins this round! Surrender your card.`;
	};
};

function takeCards() {
	if (pHand[(pHand.length - 1)].value > cHand[(cHand.length - 1)].value) {
		pPile.push(...pHand.splice(0, pHand.length), ...cHand.splice(0, cHand.length));
	} else {
		cPile.push(...pHand.splice(0, pHand.length), ...cHand.splice(0, cHand.length));
	};
...
function runWar() {
	...
	// take four cards out of each pile and push to hand.
